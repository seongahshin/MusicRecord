...
extension WriteView {
    ...
    func saveData() {
        ...
        let record = Record(date: sharedDateManager.selectedDate ?? "", records: [
            DayRecord(id: UUID(), albumImage: ImageManager.getArtworkURL(artwork: song!.artwork), songID: song!.id.rawValue, songTitle: song!.title, singer: song!.artistName, detailRecord: text)
        ])
